...
4. Greps `~/.ssh/config` for an `hpg` alias; prints a sample config block + setup hint if missing. **Does not block setup** — teammates who only want to run OpenSplat locally don't need HPG.
...
- **Mac / Linux only.** The script uses `bash`-isms (`set -euo pipefail`, `source bin/activate`, unix paths). Windows teammates should follow the manual section in `SETUP.md`.
...
Anything tooling-adjacent that a teammate would invoke by hand belongs here. Keep each script self-documenting (top-of-file comment block describing what it does, required env vars, and safe-to-re-run semantics).
