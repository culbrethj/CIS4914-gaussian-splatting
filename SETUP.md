...
Create (or edit) `C:\Users\<you>\.ssh\config`. Windows does not auto-create the `.ssh` folder, so you may need to create it manually, and set permissions so only your user can read it:
...
1. **SSH into HPG**, clone the repo into your own `/blue/cis4914/<gatorlink>/` workspace.

2. **Conda env**: build your own following `backend/experiments/faster-gs/README.md` (section "VGGT: one-time HPG setup" and the surrounding env setup). Takes ~15 min for a fresh install.

3. **Set `FASTERGS_REMOTE_ROOT` to point at your workspace**:
...
Full documentation lives in [backend/experiments/faster-gs/README.md](backend/experiments/faster-gs/README.md). That doc covers the HPG workspace layout, camera-model undistort step, shortgs paper flags, partition defaults, and troubleshooting. Read it before kicking off the matrix.
